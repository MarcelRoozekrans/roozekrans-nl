...
| `a5020d7` | `border-strong` was specified as a per-theme neutral measuring 1.42:1 and 1.91:1, while being the sole affordance on secondary buttons. `--measure` was applied to About's whole page shell rather than its prose. |
| `9022f85` | Task 13 specified `github-light`/`github-dark`, chosen for legibility and never measured — keyword 4.16:1, comment 3.68:1. Swapped to the high-contrast variants. **This amendment was initially missed from this table**, and the contract's Open Question 3 went stale for a commit; requiring every retroactive doc edit to land here is what catches that. |
| `05c4309` | MASTER's secondary-button hover rule was **unsatisfiable**: it required the border to clear 3:1 while prescribing a hover to `text-disabled`, which the same document records as 2.5:1. Raising `border-strong` to `#71717A` then made the hover a silent no-op in dark, since both tokens became the same value. |
| `1f5b1d7` | The reduced-motion guard collapsed `transition-duration` but not `translate`, so the card lift snapped rather than eased — drift item 5 was still open while MASTER claimed all nine were resolved. |
| `a14eee7` | The token guard scanned 13 files while Tailwind scanned 46, so a raw palette class in any markdown post passed CI. Worse, Tailwind scanned the guard script itself, generating the 18 palette class names in its hint table into the production stylesheet — **21% of the shipped CSS was the utilities the gate forbids**. |
| `c525e3f` | Card link targets were 20px against a 44px requirement; `scroll-margin-top` went stale at 80px when the nav grew to 101px; inline code receded in dark where chips lift; Shiki's `wrap: true` broke identifiers mid-token and wrapped mid-expression on desktop. |
